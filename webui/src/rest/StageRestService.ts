...
export const loadStages = (
  onSuccess: (stages: StageSearchViewModel[]) => void,
  onFailure: (error: Error) => void
) => {
  (async () => {
    logger.info("Loading stages.");
    try {
      const response = await axios.get(`${restConfig.ROOT_URL}/stages`);
      logger.info("Loaded stages.");
      onSuccess(response.data);
    } catch (error) {
      logger.error("Failed to load stages.", error);
      onFailure(unexpectedError);
    }
  })();
};
...
export const saveStage = async (
  ...
  try {
    const response = stage.id === null
      ? await axios.post(`${restConfig.ROOT_URL}/stages`, stage)
      : await axios.put(`${restConfig.ROOT_URL}/stages/${stage.id}`, stage);

    const savedId = response.data.id;
    logger.info(`Saved stage ${savedId}.`);
    onSuccess({...stage, id: savedId});
  } catch (error) {
